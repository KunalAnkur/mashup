...
export const useStream = ({
    ...
    //     const audioTrack = newStream.getAudioTracks()[0];
    //     const videoTrack = newStream.getVideoTracks()[0];
    //     // Don't replace tracks if we're currently seeking
    //     if (isSeekingRef.current) {
    //         console.log("[STREAM] Skipping track replacement during seek");
    //         return;
    //     }

    //     try {
    //         console.log("producer called replaceProducerTracks called", audioProducerRef.current, audioTrack, videoProducerRef.current, videoTrack);
    //         console.log("replaceProducerTracks ==", { audioTrack, videoTrack });
    //         // Only replace audio track if it's live and enabled
    //         // if (audioProducerRef.current && !audioProducerRef.current.closed && audioTrack) {
    //         //     // Check if the new track is live before replacing
    //         //     if (audioTrack.readyState === 'live' && audioTrack.enabled) {
    //         //         hasAudioTrack = true;
    //         //         await audioProducerRef.current.replaceTrack({ track: audioTrack });
    //         //         console.log("[STREAM] Audio track replaced successfully");
    //         //     } else {
    //         //         console.warn("[STREAM] Skipping audio track replacement - track not live or disabled", {
    ...
    //         // // Only replace video track if it's live
    //         // if (videoProducerRef.current && !videoProducerRef.current.closed && videoTrack) {
    //         //     // Check if the new track is live before replacing
    //         //     if (videoTrack.readyState === 'live') {
    //         //         hasVideoTrack = true;
    //         //         await videoProducerRef.current.replaceTrack({ track: videoTrack });
    //         //         console.log("[STREAM] Video track replaced successfully");
    //         //     } else {
    //         //         console.warn("[STREAM] Skipping video track replacement - track not live", {
    //         //             readyState: videoTrack.readyState
    //         //         });
    //         //     }
    //         // }
    //         const tracks = helper.getStreamTracks(newStream);
    //         const promises = tracks.map(track => {
    //             if (track.kind === 'video') return videoProducerRef.current?.replaceTrack({ track: track });
    //             if (track.kind === 'audio') return audioProducerRef.current?.replaceTrack({ track: track });
    //         });
    ...
    //     }
    // }, [isHost, roomId, socket, helper.getStreamTracks]);
    const replaceProducerTracks = useCallback(async (newStream: MediaStream) => {
