...
import { AuthHeader, Button, Input } from "../UI";
...
const AuthWrapper = ({ isModel = false }: { isModel?: boolean }) => {
    ...
    return (
        <div className="flex flex-col items-center justify-center w-full h-full bg-[#18181b] gap-20">
            <AuthHeader
                title={
            ...
            <div className="w-full max-w-md">
                {container === "login" && <LoginContainer setContainer={setContainer}  />}
                {container === "signup" && <SignupContainer setContainer={setContainer} />}
            </div>
            {!isModel &&
                <Button
                    onClick={handleBack}
