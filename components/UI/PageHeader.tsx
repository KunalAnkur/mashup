...
const PageHeader: React.FC<PageHeaderProps> = ({
  ...
  return (
    <div className="w-full flex items-center justify-between px-3 sm:px-4 md:px-6 py-2.5 sm:py-3 md:py-4 border-b border-white/10 relative z-40 flex-shrink-0">
      {/* Left Section: Back Button + Logo */}
      <div className="flex items-center gap-2 sm:gap-3 md:gap-5 min-w-0">
        <button
          onClick={onBack}
          className="flex items-center justify-center w-8 h-8 sm:w-9 sm:h-9 rounded-lg bg-white/5 hover:bg-white/10 active:bg-white/15 text-gray-300 hover:text-white transition-all duration-200 flex-shrink-0 group"
          aria-label="Go back"
        >
          <FaArrowLeft className="text-sm sm:text-base group-hover:-translate-x-0.5 transition-transform duration-200" />
        </button>
        <div className="h-4 sm:h-5 w-px bg-white/10 flex-shrink-0 hidden sm:block"></div>
